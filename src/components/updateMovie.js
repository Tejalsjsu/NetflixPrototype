...
class UpdateMovie extends Component {
  ...
  handleSearch(searchText, filterValues, page, size) {
    ...
    API.searchMovie(searchText, filterValues, page, size).then(res => {
      ...
      if (res.length > 0) {
        ...
        if (
          ...
          for (i = 0; i <= data.length - 1; i++) {
            if (
              filterValues.genre != "" &&
              data[i].genre == filterValues.genre
            ) {
            ...
            if (
              filterValues.year != 0 &&
              data[i].year == filterValues.year &&
              this.state.movieList.indexOf(data[i].title) < 0
            ) {
              this.state.movieList.push(data[i].title);
            } else if (
              filterValues.rating != "" &&
              data[i].rating == filterValues.rating &&
              this.state.movieList.indexOf(data[i].title) < 0
            ) {
              this.state.movieList.push(data[i].title);
            } else if (
              filterValues.numberOfStars != 0 &&
              data[i].numberOfStars == filterValues.numberOfStars &&
              this.state.movieList.indexOf(data[i].title) < 0
            ) {
              this.state.movieList.push(data[i].title);
            } else if (
              filterValues.actors != 0 &&
              data[i].actors == filterValues.actors &&
              this.state.movieList.indexOf(data[i].title) < 0
            ) {
              this.state.movieList.push(data[i].title);
            }
        ...
        if (this.state.movieList.length > 0) {
          for (i = 0; i <= this.state.movieData.length - 1; i++) {
            if (
              this.state.movieList.indexOf(this.state.movieData[i].title) > -1
            ) {
              console.log("KEEP!");
            } else {
  ...
  handleUpdateNav = (movieID, movieTitle) => {
    ...
    console.log(
      ...
      this.state.updateMovieData.title
    );
  ...
  render() {
    ...
    const withMorefilter =
      ...
      Object.keys(this.state.projectData).map(pd => {
        return (
          <tr key={this.state.projectData[pd]._id} onClick={self.handleClick}>
            <td key={this.state.projectData[pd].title}>
              <td>
                <Button
                  name="Update Movie"
                  bsStyle="info"
                  ...
                  onClick={() =>
                    this.handleUpdateNav(
                      this.state.movieData[pd]._id,
                      this.state.movieData[pd].title
                    )
                  }
                >
                  Update
              ...
              <td>
                <Button
                  ...
                  class="btn btn-primary "
                  onClick={() =>
                    this.handleDelete(this.state.movieData[pd]._id)
                  }
                  data-toggle="modal"
    ...
    const withKeys = this.state.movieList.map(function(item) {
      return (
        <tr value={item}>
          ...
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <td>
            <Button name="Update Movie" bsStyle="info" class="btn btn-primary ">
              Update
            </Button>
          </td>
          &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    const withfilter =
      ...
      Object.keys(this.state.movieData).map(pd => {
        return (
          <tr key={this.state.movieData[pd]._id}>
            <td key={this.state.movieData[pd]._id}>
              {this.state.movieData[pd].title}
            </td>
            ...
            <td>
              <Button
                ...
                key={this.state.movieData[pd]._id}
                class="btn btn-primary "
                onClick={() => this.handleDelete(this.state.movieData[pd]._id)}
                data-toggle="modal"
                data-target="#myDeleteModal"
              >
                Delete
                  ...
                  <div class="modal-content">
                    <div class="modal-header">
                      ...
                      <h4 class="modal-title">
                        Movie features updated with id :{" "}
                        {this.state.movieToUpdate} successfully!
                      </h4>
                    </div>
                    <div class="modal-body">
                      <p>
                        Users will now be able to view updated features for
                        movie ABC!!
                      </p>
                      <p>
                        Following features updated: {this.state.movieToUpdate}
                      </p>
                    </div>
            ...
            <div style={divStyle1} className="col-sm-3">
              ...
              <form>
                ...
                <Button
                  ...
                  onClick={() =>
                    this.handleUpdate(
                      this.state.updateMovieData.updateMovieID,
                      this.state.updateMovieData
                    )
                  }
                >
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <p>
                          {" "}
                          Please note any current ongoing activity would not be
                          affected for any user.
                        </p>
                      </div>
                      <div class="modal-footer">
                        <button
